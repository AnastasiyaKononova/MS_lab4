...
public class Main2Activity extends AppCompatActivity implements View.OnClickListener {
   ...
   static DBHelper dbHelper2;
    ...
    public void onClick(View v) {
        ...
        Log.d(LOG_TAG, " --- db v." + db2.getVersion() + " --- ");
            ...
            case R.id.btnClear:
                ...
                // добавляем 5 записей
                   /* if(version == 1) {
                    ...
                    cv.put("id_n", "1");
                    cv.put("first_name", "Андрей");
                    cv.put("middle_name", "Анатольевич");
                    cv.put("last_name", "Синичкин");
                    cv.put("date", "28.11.2016");
                    long rowID21 = db2.insert("classmates", null, cv);
                    Log.d(LOG_TAG, "row inserted, ID = " + rowID21);

                    cv.put("id_n", "2");
                    cv.put("first_name", "Кристина");
                    cv.put("middle_name", "Михайловна");
                    cv.put("last_name", "Дорошкевич");
                    cv.put("date", "28.11.2016");
                    long rowID22 = db2.insert("classmates", null, cv);
                    Log.d(LOG_TAG, "row inserted, ID = " + rowID22);

                    cv.put("id_n", "3");
                    cv.put("first_name", "Антон");
                    cv.put("middle_name", "Анатольевич");
                    cv.put("last_name", "Рекиш");
                    cv.put("date", "28.11.2016");
                    long rowID23 = db2.insert("classmates", null, cv);
                    Log.d(LOG_TAG, "row inserted, ID = " + rowID23);

                    cv.put("id_n", "4");
                    cv.put("first_name", "Марина");
                    cv.put("middle_name", "Михайловна");
                    cv.put("last_name", "Котович");
                    cv.put("date", "28.11.2016");
                    long rowID24 = db2.insert("classmates", null, cv);
                    Log.d(LOG_TAG, "row inserted, ID = " + rowID24);

                    cv.put("id_n", "5");
                    cv.put("first_name", "Станислав");
                    cv.put("middle_name", "Витальевич");
                    cv.put("last_name", "Атрохов");
                    cv.put("date", "28.11.2016");
                    // вставляем запись и получаем ее ID
                    long rowID25 = db2.insert("classmates", null, cv);
                    Log.d(LOG_TAG, "row inserted, ID = " + rowID25);
            ...
            case R.id.btnRead:
                ...
                if (c.moveToFirst()) {
                   ...
                   /* if(version2 == 1) {
                    ...
                    int lastNameColIndex = 0;
                    int middleNameColIndex = 0;
                    int firstnameColIndex = 0;

                        firstnameColIndex = c.getColumnIndex("first_name");
                        lastNameColIndex = c.getColumnIndex("last_name");
                        middleNameColIndex = c.getColumnIndex("middle_name");
                        int idColIndex = c.getColumnIndex("id");
                        int id_nColIndex = c.getColumnIndex("id_n");
                        int dateColIndex = c.getColumnIndex("date");
                        do {
                            ...
                            Log.d(LOG_TAG,
                                    "ID = " + c.getInt(idColIndex) +
                                            ...
                                            ", middle_name = " + c.getString(middleNameColIndex) +
